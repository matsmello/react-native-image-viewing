...
import { Image, Dimensions as RDimensions } from "react-native";
...
const useImageDimensions = (image) => {
    ...
    const getImageDimensions = (image) => {
        return new Promise((resolve) => {
            const { width: originalW, height: originalH } = Image.resolveAssetSource(image);
    ...
    useEffect(() => {
        getImageDimensions(image).then((dimensions) => {
            if (!isImageUnmounted) {
                let newDimensions = dimensions.height == 0 || dimensions.width == 0
